...
Cypress.Commands.add('waitForAnalysis', () => {
  ...
  cy.get('[data-testid="loading-spinner"]', { timeout: 2000 }).should(
    'be.visible'
  );
  cy.get('[data-testid="loading-spinner"]', { timeout: 30000 }).should(
    'not.exist'
  );

  // Wait for results to be displayed
  cy.get('[data-testid="analysis-results"]', { timeout: 5000 }).should(
    'be.visible'
  );
});
